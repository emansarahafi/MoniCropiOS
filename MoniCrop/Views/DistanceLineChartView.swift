...
struct DistanceLineChartView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("Distance chart")
        .accessibilityValue(data.isEmpty ? "No data" : "Latest: \(String(format: "%.2f", data.last!.1)) on \(data.last!.0.formatted(date: .abbreviated, time: .omitted))")
    }
}

struct DistanceLineChartView_Previews: PreviewProvider {
    static var previews: some View {
        let now = Date()
        let sample: [(Date, Double)] = [
            (Calendar.current.date(byAdding: .day, value: -5, to: now)!, 5.0),
            (Calendar.current.date(byAdding: .day, value: -4, to: now)!, 6.0),
            (Calendar.current.date(byAdding: .day, value: -3, to: now)!, 5.5),
            (Calendar.current.date(byAdding: .day, value: -2, to: now)!, 7.0),
            (Calendar.current.date(byAdding: .day, value: -1, to: now)!, 6.8),
            (now, 8.0)
        ]
