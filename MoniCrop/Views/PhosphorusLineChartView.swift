...
struct PhosphorusLineChartView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("Phosphorus chart")
...
struct PhosphorusLineChartView_Previews: PreviewProvider {
    static var previews: some View {
        let now = Date()
        let sample: [(Date, Double)] = [
            (Calendar.current.date(byAdding: .day, value: -5, to: now)!, 4.0),
            (Calendar.current.date(byAdding: .day, value: -4, to: now)!, 4.5),
            (Calendar.current.date(byAdding: .day, value: -3, to: now)!, 4.2),
            (Calendar.current.date(byAdding: .day, value: -2, to: now)!, 4.8),
            (Calendar.current.date(byAdding: .day, value: -1, to: now)!, 4.6),
            (now, 5.0)
        ]
